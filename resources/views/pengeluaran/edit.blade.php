        ...
        <form action="{{ route('pengeluaran.update', $pengeluaran->id_pengeluaran) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="id_unit" id="id_unit" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        @foreach ($units as $unit)
                            <option value="{{ $unit->id_unit }}" {{ $pengeluaran->id_unit == $unit->id_unit ? 'selected' : '' }}>
                                {{ $unit->nama_unit }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal" id="tanggal" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" value="{{ \Carbon\Carbon::parse($pengeluaran->tanggal)->format('Y-m-d') }}">
                </div>
            </div>

            <div class="mt-6">
                <label for="deskripsi" class="block text-sm font-medium text-gray-700">Deskripsi</label>
                <textarea name="deskripsi" id="deskripsi" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ $pengeluaran->deskripsi }}</textarea>
            </div>
                ...
                <div id="items-container">
                    @foreach ($pengeluaran->details as $index => $detail)
                        <div class="flex gap-4 mb-4 items-end">
                            <div class="flex-1">
                                <label for="item_keperluan_{{ $index }}" class="block text-sm font-medium text-gray-700">Keperluan</label>
                                <input type="text" name="items[{{ $index }}][nama_keperluan]" id="item_keperluan_{{ $index }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" value="{{ $detail->nama_keperluan }}" required>
                            </div>
                            <div class="w-24">
                                <label for="item_jumlah_{{ $index }}" class="block text-sm font-medium text-gray-700">Jumlah</label>
                                <input type="number" name="items[{{ $index }}][jumlah]" id="item_jumlah_{{ $index }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" min="1" value="{{ $detail->jumlah }}" required>
                            </div>
                            <div class="w-28">
                                <label for="item_harga_{{ $index }}" class="block text-sm font-medium text-gray-700">Harga</label>
                                <input type="number" name="items[{{ $index }}][harga]" id="item_harga_{{ $index }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" min="0" value="{{ $detail->total / $detail->jumlah }}" required>
                            </div>
                            <div>
            ...
            <div class="mt-6">
                ...
                <a href="{{ route('pengeluaran.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded ml-2">
                    Batal
                </a>
            </div>
        </form>
