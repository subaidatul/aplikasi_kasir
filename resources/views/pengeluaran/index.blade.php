                    ...
                    @foreach ($pengeluarans as $pengeluaran)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex space-x-2">
                                ...
                                <a href="{{ route('struk.show', ['jenis' => 'pengeluaran', 'id' => $pengeluaran->id_pengeluaran]) }}"
                                    target="_blank" class="text-blue-600 hover:text-blue-900">
                                    Cetak Struk
                                </a>
                                
                                {{-- Tambahkan tombol hapus ini --}}
                                <form action="{{ route('pengeluaran.destroy', $pengeluaran->id_pengeluaran) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pengeluaran ini?');">
                                    @csrf
                                    @method('DELETE')
